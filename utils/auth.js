...
let datamanual = document.getElementById("datamanual");
let manualpay = document.getElementById("manualpay");
let activemanualpay = document.getElementById("activemanualpay");
...
async function fortest() {
  try {
    const response = await requestData(
      "https://earnsmart.seosblog.com/?action=register",
      "POST",
      { some: "data" }
    );
...
document.addEventListener("click", (event) => {
  ...
  if (
    navbar.classList.contains("navhelper") &&
  ...
  try {
    ...
    if (response.resultcode) {
      ...
      function proptupdate() {
        alert(
          `Hi ${uname}, Please Update Your Registration Details to proceed.`
        );
        let myemail = null;
        let myphone = null;
      ...
      document
        ...
        .forEach((el) => (el.innerHTML = target));
      document
        .querySelectorAll("#reward")
        .forEach((el) => (el.innerHTML = reward));
      document
        .querySelectorAll("#yourpoints")
        .forEach((el) => (el.innerHTML = progress));
      document
        .querySelectorAll("#rempoint")
        .forEach((el) => (el.innerHTML = remaining));

      document
        .querySelectorAll("#percent")
        .forEach((el) => (el.innerHTML = percent + "%"));
      ...
      if (status == 1 && actbal <= 0) {
        ...
        if (activemanualpay) {
          activemanualpay.style.display = "none";
        }
      ...
      if (manualpayment) {
        if (manualpay) {
          ...
          if (activemanualpay){
          activemanualpay.style.display = "grid";
...
let activatepay = document.getElementById("activatepay");
...
function justcon() {
  ...
  async function teamGet() {
    try {
      const response = await requestData(
        `${baseUrl}myDownlines&level=${lastDigit}`
      );
      ...
      mylevel.innerHTML =
        response.data["Currency"] + " " + response.data["Earned"];
...
if (youtubediv) {
  ...
  function populateVideos(videos) {
    ...
    videos.forEach((video) => {
      ...
      if (videoId) {
        ...
        document
          .getElementById(`startButton-${video.No}`)
  ...
  async function payoutvideo(vid) {
    try {
      const response = await requestData(`${baseUrl}payyoutube`, "POST", {
        vid: vid,
      });
      if (Array.isArray(response.info) && response.info.length > 0) {
        notify(response.info);
...
if (addiv) {
  ...
  function populteads(videos) {
    ...
    videos.forEach((video) => {
      ...
      videoDiv.innerHTML = `
                        ...
                        <div class="adimg">
                            <img src="${mySource}modules/networking/${
        video.v_url
      }"  alt="">
                        </div>
                        ...
                        <i class="fa-solid fa-thumbs-up ${
                          ...
                          JSON.parse(video.v_status["data"])["liked"] &&
                          `amclicked`
                        }" id="likeBtn-${video.v_id}"></i>
                        <textarea name="message" ${
                          video.v_status["res"] === true && `disabled`
                        } placeholder="Add Comment..." id=""  class="${
        video.v_status["res"] === true && `commented`
      }"
                        maxlength="60">${
                          video.v_status["res"] === true
                            ? JSON.parse(video.v_status["data"])["message"]
                            : ""
                        }</textarea>
      ...
      if (!video.v_status["res"]) {
        document
          .getElementById(`startButton-${video.v_id}`)
          .addEventListener("click", function () {
  ...
  async function payoutvideo(vid) {
    try {
      const response = await requestData(`${baseUrl}paytiktok`, "POST", {
        vid: vid,
      });
      if (Array.isArray(response.info) && response.info.length > 0) {
        notify(response.info);
      ...
      document
        ...
        .addEventListener("click", function () {
          ...
          if (video.v_status === false) {
            document.getElementById(`overlay-${video.No}`).style.display =
              "none";
            setTimeout(() => {
              payoutvideo(video.v_id);
    ...
    try {
      ...
      function formatter(number) {
        ...
        if (!isNaN(numericValue)) {
          ...
          return (
            formattedNumber.replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " " + ucur
          );
        } else {
      ...
      if (response.data.status) {
        commentearn.innerHTML =
          "Congrats! You've earned from your free spin. Try your luck with unlimited spins in the Casino Spin!";
      }
...
if (casinodiv) {
  ...
  let myitems = [1600, 5, 0.2, 0.8, 0.5, 800, 0, 0.5, 0.6, 200, 1.3, 2, 1.6];
  ...
  async function draw() {
    ...
    for (let i = 0; i < items.length; i++, startDeg += step) {
      ...
      let displayText = Number.isInteger(rannumber)
        ? rannumber
        : `x${rannumber}`;
...
function myalert(msg) {
  ...
  cont.className =
    "w-[96%] fixed top-6 p-2 lg:w-96 bg-purple-700 text-white rounded-md shadow-md animate-slide-down border overflow-hidden z-50";
  cont.id = "myalert";
  cont.innerHTML = `
        <p class="p-2">${msg}</p>
  ...
  document
    ...
    .addEventListener("click", () => closeAlert(false));
  document
    .getElementById("okayBtn")
    .addEventListener("click", () => closeAlert(true));
